...
### Факт 1 — это не внутренний IP

`172.9.4.222` — **публичный** адрес. Приватный диапазон по RFC 1918: `172.16.0.0/12`.  
`172.9.x.x` за его пределами. WHOIS:
...
### Факт 2 — устройство реагирует на этот адрес иначе

Запрос на DVR с заголовком `Host: 172.9.4.222` возвращает **HTTP `000`** — соединение обрывается без единого байта ответа. Обычный запрос возвращает `401 Unauthorized`. Сервер **знает** об этом адресе и молча дропает соединение. Почему прошивка DVR знает о конкретном IP в AT&T-пространстве?

---

### Факт 3 — AT&T и NSA: задокументированное партнёрство

AT&T — не просто телеком. Это **главный технический партнёр АНБ США** по массовой слежке, что подтверждено рассекреченными документами (Эдвард Сноуден, 2013, ProPublica/NYT, 2015):

- **Room 641A** — секретный зал в AT&T-хабе Сан-Франциско, где стоит оборудование NSA. Оптические сплиттеры снимают **полную копию** всего интернет-трафика, проходящего через узел
- AT&T установила аналогичное оборудование в **17 своих интернет-хабах** по всей стране. NSA расширило присутствие до **8 «крепостей»**: Атланта, Чикаго, Даллас, Лос-Анджелес, Нью-Йорк, Сан-Франциско, Сиэтл, Вашингтон
- С 2011 года AT&T передавала АНБ **1,1 миллиарда записей о звонках ежедневно**
- Программа **Fairview** (2003): в первые месяцы — 400 миллиардов метаданных, затем свыше миллиона email в день в штаб-квартиру NSA в Форт-Мид
- Внутренние документы NSA характеризуют AT&T как партнёра с **«крайней готовностью помочь»**

AT&T получила ретроактивный иммунитет от судебного преследования через FISA Amendments Act 2008 — принятый уже после того, как EFF подала иск Hepting v. AT&T за незаконную слежку.

---

### Факт 4 — Hikvision и китайское государство

Hikvision — не частная компания:

- Мажоритарный акционер: **CETC** (China Electronics Technology Group Corporation) — государственный ВПК КНР, подчиняется Госсовету
- **2019**: внесена в Entity List США — запрет на покупку американских компонентов — за поставку систем распознавания лиц для лагерей в **Синьцзяне**
- **2022**: FCC запретила продажу оборудования Hikvision в США как «неприемлемую угрозу национальной безопасности»
- Множество штатов (Джорджия, Нью-Гэмпшир, Нью-Джерси и др.) ввели собственные запреты
- **CVE-2017-7921**: задокументированный бэкдор в камерах 2014–2017 гг. Обход аутентификации через URL-параметр. Присутствовал три года. Hikvision: «отладочный код»
...
```
  ┌─────────────────────────────────────────────────────────────┐
  │  Миллионы DVR/NVR Hikvision в домах, банках, офисах мира   │
  │                         (24/7 online)                       │
  └──────────────────────────┬──────────────────────────────────┘
                             │  firmware hardcode: 172.9.4.222
                             ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  AT&T Enterprises, LLC  (172.0.0.0/12)                      │
  │  Dallas, TX — backbone интернет-инфраструктура США          │
  │                                                              │
  │  ┌─────────────────────────────────────────────────────┐    │
  │  │  Room 641A + 17 хабов + 8 «крепостей»               │    │
  │  │  NSA Fairview — полная копия трафика                 │    │
  │  │  1.1 млрд записей/день → Fort Meade                  │    │
  │  └─────────────────────────────────────────────────────┘    │
  └──────────────────────────────────────────────────────────────┘
              ↑                              ↑
   Китайское государство               Правительство США
   (CETC → Hikvision)                  (NSA → AT&T)
```
...
Два правительства — китайское и американское — независимо друг от друга выстроили инфраструктуру слежки. Один вшивает IP в прошивку камер. Другой перехватывает весь трафик через этот IP на уровне backbone.

Случайно ли китайская государственная компания прописала в своей прошивке адрес из блока главного партнёра NSA — или нет, установить без расшифровки прошивки и сетевого дампа невозможно. Но архитектура создаёт **технически работоспособный канал** наблюдения — вне зависимости от того, был ли он создан намеренно.

> *Это технический анализ публично доступных данных. Прямых доказательств намеренного сотрудничества между Hikvision и AT&T/NSA не обнаружено. Всё вышеизложенное — задокументированные факты плюс логические выводы из них.*
